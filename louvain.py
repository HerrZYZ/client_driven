...
class Louvain(BaseClustering, VerboseMixin):
    ...
    * Traag, V. A., Van Dooren, P., & Nesterov, Y. (2011).
    ...
    """
    def __init__( self, resolution: float = 1, action_ratio: float = 0.2, future_leak_ratio: float = 0.5, localepochlength: float = 1, bigepochlength: float = 200, threshold: float = 0.5, difficulty: float = 10, num_shards : float = 100, loop_thres : float = 50, modularity: str = 'dugue', tol_optimization: float = 1e-3,
        ...
        self.future_leak_ratio = future_leak_ratio
        self.localepochlength = localepochlength
        self.bigepochlength = bigepochlength
        self.threshold = threshold
        self.difficulty = difficulty
        self.num_shards = num_shards
        self.loop_thres = loop_thres
    ...
    def _optimize(self, adjacency_norm, probs_ou, probs_in):
        ...
        return fit_core(self.resolution, self.action_ratio, self.future_leak_ratio, self.localepochlength, self.bigepochlength, self.threshold, self.difficulty, self.num_shards, self.loop_thres, self.tol, node_probs_ou, node_probs_in, self_loops, data, indices, indptr)
